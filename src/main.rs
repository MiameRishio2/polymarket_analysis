...
use polymarket_analysis::{
    config, create_router as create_menu_router, create_sqlite_router, Storage,
};
...
async fn main() {
    ...
    let sqlite_router = create_sqlite_router("data/menu.db".to_string());

    // Combine routers
    let app = menu_router.merge(sqlite_router);
